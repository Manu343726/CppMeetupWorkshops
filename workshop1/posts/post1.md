...
> _"Any sufficiently complex C++ code is indistingishable from trolling"_
> Arthur C. Clarke
...
So instead of writing code that is compiled and does something at run-time (i.e. represents some actions to be done at run-time), we write code (_meta-code?_) that generates code. Let me show you a simple example:
    ...
    * * *
    ...
    So if we write a template `foo` <span class="b2" style="color: #242424;">parameterized</span> with a type parameter:
    ...
    * * *
    ...
    <pre>`using t = typename identity<int>::type; // t is int
    `</pre>
    ...
    <pre>`using t = typename std::remove_reference<int&>::type; //t is int
    `</pre>
    ...
    * * *
    ...
    <pre>`using t = typename throw_stars<int********>::type; //t is int 
    `</pre>
